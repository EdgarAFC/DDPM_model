...
def rayleigh_pdf():
    ...
    # hist, bins = np.histogram(flattened_data, bins=1000)  # Adjust the number of bins as needed
    ...
    # plt.grid(True)
    # plt.show()
    hist, bins = np.histogram(flattened_data, bins=1000)
    # Generate x values
    x_values = np.linspace(0, 1, 1000)
    # plt.plot(x_values, hist/hist.sum(), color='blue', label='Rayleigh PDF')
    # plt.show()
...
def main():
    ...
    for ep in range(trained_epochs+1, n_epoch+1):
        ...
        for x, y in train_loader:  # x: images
            ...
            # perturb data
            # Sample from the PDF
            num_samples = BATCH_SIZE * 1 * 128 * 800  # Total number of samples
            hist,x_values=rayleigh_pdf()
