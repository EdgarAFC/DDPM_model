...
################################
file_loss = open("/mnt/nfs/efernandez/projects/DDPM_model/log_sampling_w1_cola.txt", "w")
file_loss.close()
################################
def write_to_file(input): 
    with open("/mnt/nfs/efernandez/projects/DDPM_model/log_sampling_w1_cola.txt", "a") as textfile: 
...
def main():
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    model_dir='/mnt/nfs/efernandez/trained_models/DDPM_model/v10_TT_50epoch'
    ...
    # print(f'Test_mse: {sum(mse_loss)/len(mse_loss)}')
        ...
        for id, sample in enumerate(generated_samples):
            ...
            write_to_file(name)
            np.save(save_dir/name, sample.cpu().detach().numpy())
